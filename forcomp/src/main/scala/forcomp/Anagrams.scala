...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case (char, charCount) :: xs =>
      for {
        expanded <- (0 to charCount).map((char, _)).toList
        comb <- combinations(xs)
      } yield (expanded :: comb).filter(x => x._2 != 0)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.toMap.foldLeft(x.toMap)({ (x, y) =>
      x.updated(y._1, x(y._1) - y._2 )
