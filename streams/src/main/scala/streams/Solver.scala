...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    for {
      (pos, move) <- b.legalNeighbors.toStream
    } yield (pos, move :: history)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moveList) #:: xs =>
      val expanded = newNeighborsOnly(neighborsWithHistory(block, moveList), explored)
      (block, moveList) #:: from(xs #::: expanded, explored + block)
  }
  /**
